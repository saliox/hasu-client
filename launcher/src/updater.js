// Auto-update — même canal éprouvé que Cap Hub : un manifeste launcher/version.json
// (version, url, sha256, notes) lu en raw sur GitHub, installeur téléchargé, VÉRIFIÉ
// par empreinte SHA-256, puis lancé en silencieux.
...
const MANIFEST_URL = 'https://raw.githubusercontent.com/saliox/hasu-client/main/launcher/version.json';
...
// L'installeur ne peut venir QUE des releases de notre dépôt.
...
async function downloadWithProgress(r, total, onProgress) {
  if (!r.body || typeof r.body.getReader !== 'function') {
    ...
    if (buf.length > MAX_INSTALLER) throw new Error('Installeur trop volumineux — refusé.');
    return buf;
  }
...
export async function applyUpdate(appQuit, onProgress) {
  ...
  try {
    ...
    // à nom ALÉATOIRE (mkdtemp), comme Cap Hub. Des chemins fixes/prévisibles dans %TEMP%
    // permettraient à un autre processus (même utilisateur) de réécrire l'installeur vérifié
    // entre l'écriture et l'exécution (TOCTOU), contournant la vérification SHA-256.
    const workDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hasulauncher-upd-'));
