// Connexion Microsoft OFFICIELLE (device code) : Microsoft -> Xbox Live -> XSTS ->
// Minecraft. Même flux et même code éprouvé que Cap Hub / snipe-mc. Les identifiants
// ne transitent jamais par nous : l'utilisateur valide sur microsoft.com, on ne
// manipule que des jetons. L'ID d'application Azure (public client, scope
// XboxLive.signin, approuvé Minecraft via https://aka.ms/mce-reviewappid) est fourni
// dans les réglages.
...
export async function requestDeviceCode(clientId) {
  if (!clientId) throw new Error('Azure Client ID manquant (Réglages → Compte).');
...
function deviceCodeError(body) {
  ...
  if (/AADSTS700016/.test(desc)) return 'Client ID introuvable côté Microsoft : vérifie l’Azure Client ID (Réglages → Compte).';
...
// permet d'interrompre. Renvoie { msRefreshToken, accessToken, expiresAt, profile }.
...
async function microsoftToSession(clientId, msTok, prevRefresh) {
  ...
  // Si Microsoft ne renvoie pas de nouveau refresh token, on garde le précédent.
...
export async function getProfile(accessToken) {
  ...
  return r.json(); // { id, name, skins, capes }
}
